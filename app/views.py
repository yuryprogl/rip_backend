...
@api_view(["POST"])
def set_async_result(request, forecast_id):
    """
    22-й МЕТОД: Callback для асинхронного сервиса.
    Принимает результаты расчета и сохраняет их в М-М.
    """
    # 1. Псевдо-авторизация по токену (8 байт)
    ...
    # 2. Получаем данные (список: [{element_id: 1, weight: 0.5}, ...])
    ...
    for item_data in results:
        ...
        # Ищем запись м-м и обновляем
        ElementForecast.objects.filter(forecast=forecast, element_id=element_id).update(weight=weight)
...
def update_status_admin(request, forecast_id):
    ...
    if request_status == 3:
        # Вместо расчета calc(), мы отправляем запрос в Go сервис
        ...
        data_to_send = {
            ...
            "elements": [
                {"element_id": item.element.id, "temperature": item.temperature} 
                for item in items
            ]
        }

        try:
            # Отправляем запрос в асинхронный сервис (имя контейнера 'go-service')
            # Таймаут маленький, так как мы просто ставим задачу
            requests.post("http://go-service:8080/calc", json=data_to_send, timeout=1)
        except:
            # Игнорируем ошибки сети, так как это демо
            pass
